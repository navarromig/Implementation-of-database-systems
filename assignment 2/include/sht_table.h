...
typedef struct {
    char primary_index[20] ; 
    int type_file ;           // 1 if heap file , 2 if hash file, 3 if sht file
    int fileDesc ;            // file descriptor
    int stored_block ;        // block where file metadata are stored
    int block_capacity ;      // number of records that can be stored in block
    int hash_table[20]; 
    int numBuckets;
} SHT_info;
...
typedef struct {
    int id ;         // id of block
    int records ;    // number of current records
    int next_block ; // id of next block (overflow)
} SHT_block_info;
